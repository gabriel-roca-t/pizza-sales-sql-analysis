...
-- Q3: Seating occupation per hour 
-- Assumes 5 tables with 4 seats each (20 seats total).
...
WITH hourly_occupancy AS (
    ...
    FROM orders o
    GROUP BY EXTRACT(HOUR FROM o.time::TIME)
)
SELECT 
    hour_of_day,
    tables_occupied,
    ROUND((tables_occupied::decimal / 5) * 100, 2) AS table_occupation_pct,
    ROUND((tables_occupied::decimal * 4 / 20) * 100, 2) AS seat_occupation_pct
